...
#[derive(Debug)]
struct Lexer<'a> {
    lower: implementation::LowerLexer<'a>,
...
impl<'a> Lexer<'a> {
    pub(crate) fn new(input: &'a str) -> Lexer<'a> {
        Lexer {
            lower: implementation::LowerLexer::new(input),
            upper: None,
        }
    }
...
mod tests {
    ...
    fn scan_spans(input: &str) -> impl Iterator<Item = (Token, Span)> {
        let mut result: Vec<(Token, Span)> = Vec::new();
        let mut scanner = Lexer::new(input);
        while let Some(token) = scanner.next() {
            let span: Range<usize> = scanner.span();
    ...
    fn scan_tokens_only<'a>(input: &'a str) -> Vec<Token> {
        scan_spans(input).map(|(token, _span)| token).collect()
    }
